...
def run_server():
    ...
    while True:
        ...
        server_id_plus_first = client_socket.recv(1024).decode() # recieve 'X' or 'O'
        ...
        if first == '1':
            print('\nClient goes first')
            current_board = gamefunctions.moveSecond(client_socket, server_id, current_board)
        else:
            print('\nServer goes first')
            current_board = gamefunctions.moveFirst(client_socket, server_id, current_board)
